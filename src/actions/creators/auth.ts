...
export const loginSuccess = (token: Token) => action(AuthActions.LOGIN_SUCCESS, token)
...
export const login = (dispatch: Dispatch<any>, graphQLService: GraphQLService) => (username: string, password: string) => {
    ...
    graphQLService.login(username, password).then(r => {
...
export const loggedOut = () => action(AuthActions.LOGGED_OUT)
...
export const autoRefreshFailed = () => action(AuthActions.AUTO_REFRESH_FAILED)
